---
title: 用模板编写网页应用
date: 2021-03-05 18:02:36
permalink: /golang/book-notes/the-way-to-go/c2c8fdc26aeec/
author: 
  name: unknwon
...
- 必须有一种机制把 `Page` 结构体数据插入到网页的标题和内容中，可以利用 `template` 包通过如下步骤完成：
	...
	```
	把要插入的数据结构字段放在 `{{` 和 `}}` 之间，这里是把 `Page` 结构体数据 `{{` 加 `.Title |html` 加 `}}` 和 `{{` 加 `printf "%s" .Body |html` 加 `}} 插入页面（当然可以是非常复杂的 html，但这里尽可能地简化了，以突出模板的原理。）（`{{` 加 `.Title |html` 加 `}}` 和 `{{` 加 `printf "%s" .Body |html` 加 `}} 语法说明详见后续章节）。
