          ...
          <dl v-else-if="type(item) === 'object'" :key="index">
            ...
            <dt>
              ...
              <template v-for="(c, i) in item.children">
                <dl v-if="type(c) === 'array'" class="inline">
                  <dt>
					<router-link :to="c[2]" :key="i">{{`${index+1}-${i+1}. ${c[1]}`}}</router-link>
				  </dt>
                </dl>
                <!-- 三级目录 -->
                <dl v-else-if="type(c) === 'object'" :key="i" class="sub-cat-wrap">
                  <dt :id="anchorText = c.title">
                    <a :href="`#${anchorText}`" class="header-anchor">#</a>
                    {{`${index+1}-${i+1}. ${c.title}`}}
                  </dt>
				  <dt>
					  <template v-for="(cc, ii) in c.children">
						  <dl v-if="type(cc) === 'array'" class="inline">
							  <dt>
								  <router-link :to="cc[2]" :key="ii">{{`${index+1}-${i+1}-${ii+1}. ${cc[1]}`}}</router-link>
							  </dt>
						  </dl>
						  <!-- 四级目录 -->
						  <dl v-else-if="type(cc) === 'object'" :key="ii" class="sub-cat-wrap">
							  <dt :id="anchorText = cc.title">
							    <a :href="`#${anchorText}`" class="header-anchor">#</a>
...
export default {
  ...
  created () {
    this.getPageData()
	console.log(this.getCatalogueList())
  ...
  methods: {
    ...
    getCatalogueList () {
	  ...
	  if(key.indexOf("/")> -1){
	  	let keys = key.split("/")
	  	let plist = sidebar[`/${keys[0]}/`]
		console.log("plist:",plist)
	  	for (let i = 0; i < plist.length; i++) {
			console.log("#####",plist[i].title)
			console.log("$$$$$",keys[1])
			console.log(keys[1].indexOf(plist[i].title)> -1)
	  		if(keys[1].indexOf(plist[i].title)> -1){
	  			catalogueList = plist[i].children
	  		}
	  	}
	  	//catalogueList = sidebar[`/${keys[0]}/`][0].children
	  }else {
