...
permalink: /golang/book-notes/the-way-to-go/dde97d65420ed/
author: 
  name: unknwon
...
如果只是想简单地把 `Execute()` 方法的第二个参数用于替换，使用 `{{` 加 . 加 `}}`。

当在浏览器环境中进行这些步骤，应首先使用 `html` 过滤器来过滤内容，例如 `{{` 加 `html .` 加 `}}`， 或者对 `FieldName` 过滤：`{{` 加 ` .FieldName |html ` 加 `}}`。
...
点号（`.`）可以在 Go 模板中使用：其值 `{{` 加 . 加 `}}` 被设置为当前管道的值。
...
`range-end` 结构格式为：`{{` 加 `range pipeline` 加 `}} T1 {{` 加 `else` 加 `}} T0 {{` 加 `end}}`。
...
预定义函数也在 [15.6节](15.6.md) 中使用：`{{` 加 ` printf "%s" .Body|html ` 加 `}}`，否则字节切片 `Body` 会作为数字序列打印出来。
